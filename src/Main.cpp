        ...
        if (!g_messaging->RegisterListener([](MessagingInterface::Message* message) {
                switch (message->type) {
                    ...
                    case MessagingInterface::kPostLoad:  // Called after all plugins have finished running
                        ...
                        if (GetMessagingInterface()->RegisterListener(nullptr, MergeMapperPluginAPI::ModMessageHandler))
                            logger::info("Successfully registered SKSE listener {} with buildnumber {}",
                                         MergeMapperPluginAPI::MergeMapperPluginName, g_interface001.GetBuildNumber());
                        else
